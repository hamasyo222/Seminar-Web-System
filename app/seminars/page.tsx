      ...
      {seminars.length === 0 ? (
        ...
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          {seminars.map((seminar) => {
            const nextSession = seminar.sessions[0]
            const confirmedOrders = nextSession?.confirmedOrdersCount ?? 0
            const remainingSeats = nextSession ? Math.max(nextSession.capacity - confirmedOrders, 0) : 0
                  ...
                  {nextSession && (
                    <div className="border-t pt-4 space-y-2">
                      <div className="text-sm text-gray-600">
                        次回開催: {formatJST(nextSession.startAt)}
                      </div>
                      <div className="flex items-center justify-between text-sm">
                        <span className="flex items-center text-gray-600">
                          <MapPin className="w-4 h-4 mr-1" />
                          {getSessionFormat(nextSession.format)}
                        </span>
                        <span className="flex items-center text-gray-600">
                      ...
                      {nextSession.ticketTypes.length > 0 && (
                        <div className="text-sm font-semibold text-blue-600">
                          ¥{nextSession.ticketTypes[0].price.toLocaleString()}〜
                        </div>
                      )}
                    </div>
                  )}
